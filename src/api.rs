...
use rocket::{fairing::AdHoc, http::{CookieJar, Status}, serde::json::Json, Request};
use rocket_dyn_templates::{context, Template};
use ::sysinfo::{Disks, RefreshKind, System};

use crate::{ read_dirs, read_files, utils::{get_bool_cookie, get_theme, is_restricted}, Config, Disk, MirrorFile, Sysinfo };
...
#[get("/listing/<file..>")]
async fn listing(file: PathBuf, jar: &CookieJar<'_>, config: &rocket::State<Config>) -> Result<Json<Vec<MirrorFile>>, Status> {
...
#[get("/iframe/<file..>")]
async fn iframe(file: PathBuf, jar: &CookieJar<'_>, config: &rocket::State<Config>) -> Result<Template, Status> {
    ...
    Ok(Template::render("iframe", context! {
        path: path,
        dirs: dir_list,
        theme: get_theme(jar),
...
fn sysinfo(jar: &CookieJar<'_>) -> Result<Json<Sysinfo>, Status> {
    if crate::utils::is_logged_in(&jar) {
        ...
        sys.refresh_specifics(RefreshKind::without_processes(RefreshKind::without_cpu(RefreshKind::everything())));
            ...
            if disk.total_space() != 0 {
                disks.push(
                    Disk { 
                        ...
                        used_space_readable: format_size(disk.total_space() - disk.available_space(), DECIMAL), 
...
async fn index() -> Json<MirrorInfo> {
    Json(MirrorInfo { version: env!("CARGO_PKG_VERSION").to_string() })
}

#[catch(default)]
fn default(status: Status, _req: &Request) -> Json<Error> {
    Json(Error { message: format!("{}", status) })
}

pub fn build_api() -> AdHoc {
    AdHoc::on_ignite("API", |rocket| async {
        rocket.mount("/api", routes![index, listing, sysinfo, iframe])
